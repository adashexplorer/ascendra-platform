...
coordinates the GDPR erasure saga (ADR-008). It deliberately does NOT perform
...
| FR-20 | Account data export + hard deletion across services | owner (erasure saga coordinator) |
...
| POST `/api/privacy/export` | Async account data export bundle | user (self) |
...
| publishes | EVT-UserErasureAcked | for its own purge (resume objects, profile rows) as one saga participant |
